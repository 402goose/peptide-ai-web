  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center px-4 py-8">
        ...
        <div className="grid w-full max-w-3xl gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {GOALS.map((goal) => (
            <button
              ...
              className={cn(
                "group flex flex-col items-start gap-3 rounded-xl border-2 p-4 text-left transition-all hover:scale-[1.02]",
                goal.bgColor,
                goal.borderColor,
                "hover:shadow-md"
              )}
            >
              <div className={cn("flex h-10 w-10 items-center justify-center rounded-lg", goal.bgColor)}>
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                {goal.peptides.map(p => (
                  <span key={p} className="text-xs px-2 py-0.5 rounded-full bg-white/60 dark:bg-black/20 text-slate-600 dark:text-slate-300">
                    {p}
                  </span>
                ))}
              </div>
      ...
      <div className="mt-8 flex flex-col items-center gap-3">
        <div className="h-px w-32 bg-slate-200 dark:bg-slate-700" />
        <button
          onClick={onSkip}
